  ...
  try {
    ...
    const query = `
      ...
      FROM 
        DECHARGERCOURS
      WHERE 
        dateHeureDecharg >= DATEADD(HOUR, -1, GETDATE())
        AND etatBroyage = 0
      ORDER BY 
        dateHeureDecharg DESC;
    `;
...
app.get('/camionsDechargerCoursDerniereHeure', async (req, res) => {
  try {
    ...
    const query = `
      SELECT id, veCode, poidsP1, poidsTare, poidsP2, 
             poidsNet, dateHeureP1, dateHeureDecharg, dateHeureP2, 
             parcelle, techCoupe, agentMatricule, etatBroyage 
      FROM DECHARGERCOURS
      WHERE dateHeureDecharg >= DATEADD(HOUR, -1, GETDATE())
      AND etatBroyage = 0
      ORDER BY dateHeureDecharg DESC;
    `;
    const result = await pool.request().query(query);
...
//Route pour enregistrer les tas broyé
app.post('/addTasInTableCanne', async (req, res) => {
  try {
    ...
    // Vérifier si un enregistrement existe déjà pour la date et l'heure spécifiées
    const checkQuery = `
      SELECT * 
      FROM TABLECANNE 
    ...
    if (existingRecord.recordset.length > 0) {
      // Si l'enregistrement existe déjà, ajouter le poids du tas broyé au tonnageCannaBroyerParTas
      query = `
        UPDATE TABLECANNE 
      ...
      query = `
        INSERT INTO TABLECANNE (TonnageCanneBroyerParTas, dateBroyage, heureBroyage)
        VALUES (@tasPoids, @currentDate, @currentHour);
      `;
    }
...
app.post('/updateNombreTas', async (req, res) => {
  try {
    ...
    // Requête SQL pour mettre à jour le nombre de tas dans la table LIGNE
    const updateQuery = `
      UPDATE LIGNE 
        ...
        for (const canne of cannes) {
          ...
          if (canne.etatSynchronisation) {
              ...
              await request
                  ...
                  .query(`
                      UPDATE TABLECANNE
                      SET TonnageCanneBroyerParTas = @tasPoidsCanneBroyerParTas
              ...
              await request
                  ...
                  .query(`
                      ...
                      VALUES (@tasPoidsCanneBroyerParTas, @AnneeTableCanne, @dateDecharg, @heureDecharg)
                  `);
          }
