...
router.get('/camionsEnAttente', async (req, res) => {
    try {
        ...
        const query = `
            SELECT VE_CODE, PR_CODE, TN_CODE, PS_CODE, PS_TECH_COUPE, PS_POIDSP1, PS_DATEHEUREP1
            FROM F_PREMPESEE
            WHERE PR_CODE = '1'
            ORDER BY PS_DATEHEUREP1 ASC;
        `;
...
router.get('/getPoidsTare', async (req, res) => {
    try {
        ...
        const query = `
            SELECT TOP 1 PS_POIDSP2
            FROM F_PESEE
            WHERE VE_CODE = @veCode AND PR_CODE = 1
